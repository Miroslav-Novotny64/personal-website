...
Expertov is a sports prediction application developed as my graduation thesis. It is a comprehensive SaaS platform designed for predicting sports results (a so-called "prediction game"). The project was born from a real-world client requirement for a modern, scalable replacement for an existing, outdated solution.
...
This also meant manual entry of results, which led to delays when evaluating individual predictions.

#### Solution

The client typically sourced match information from livesport.cz. I therefore decided it would be ideal if the application could automatically fetch match data from this website. Livesport, known internationally as Flashscore, does not offer a public API for data retrieval, so I opted for a web scraping approach.
...
The application's design was problematic not only visually, but also in terms of usability. While everything necessary could be done within the app, it was not done in an intuitive way — using it felt more like working in Excel than using a modern application.
...
- **Mobile-first Design:** The application is fully responsive and optimized for comfortable predicting on mobile devices.
