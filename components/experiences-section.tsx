...
export function ExperiencesSection() {
    ...
    const Icon =
      item.type === "education"
        ...
        : item.type === "freelance"
          ...
          : Briefcase;
    const pathname =
      item.type === "education" ? "/education/[slug]" : "/experience/[slug]";
    ...
    return (
      ...
      >
        ...
        <div className="flex items-start gap-4">
          ...
          <div
            className={cn(
              "relative flex h-[46px] w-[46px] shrink-0 items-center justify-center bg-transparent overflow-hidden",
              item.id === "educanet" ? "rounded-full" : "rounded-xl",
            )}
          >
            {item.logo ? (
              <Image
        ...
        <div className="flex flex-col gap-0.5 mt-1">
          {orgOrSubtitle && (
            <div
              className={cn(
                "text-[13px] font-mono font-bold leading-tight",
                item.type === "freelance"
                  ? "text-emerald-500/90"
                  : "text-primary/90",
              )}
  ...
  const jobs = ALL_ITEMS.filter(
    (i) => i.type === "work" || i.type === "internship",
  );
